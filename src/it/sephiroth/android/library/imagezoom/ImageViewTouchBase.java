...
public class ImageViewTouchBase extends ImageView implements IDisposable {
	 ...
	 * @param reset
	 *          - if true the image bounds will be recreated, otherwise the old
	 *          {@link Matrix} is used to display the new bitmap
	...
	protected void _setImageDrawable(final Drawable drawable,
			final boolean reset, final Matrix initial_matrix, final float maxZoom) {
	...
	protected void getProperBaseMatrix2(Drawable bitmap, Matrix matrix) {
		...
		float scale = Math.min(widthScale, heightScale);
		matrix.postScale(scale, scale);
		matrix.postTranslate((viewWidth - w * scale) / MAX_ZOOM, (viewHeight - h
				* scale)
				/ MAX_ZOOM);
	}
	...
	protected void zoomTo(float scale, float centerX, float centerY,
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				float newScale = (float) mEasing.easeInOut(currentMs, 0, deltaScale,
						durationMs);
